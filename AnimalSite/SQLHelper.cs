    ...
    public static DataTable SelectData(string sql)
    {
        ...
        using (SqlConnection conn = new SqlConnection(SQLHelper.connectionString))
        {
            ...
            using (SqlCommand comm = new SqlCommand(sql, conn))
            {
                try
                {
                    //open connection with the DB - ôúéçú ä÷éùåø ìáñéñ äðúåðéí 
                    conn.Open();
                    comm.CommandTimeout = 0;
                    ///execute the query -  áéöåò ùàìúà
                ...
                {
                    //close connection -  ñâéøú ä÷éùåø ìáñéñ äðúåðéí
                    conn.Close();
                }
            }
        }
        ...
        using (SqlConnection conn = new SqlConnection(SQLHelper.connectionString))
        {
            ...
            using (SqlCommand comm = new SqlCommand(sql, conn))
            {
                try
                {
                    ...
                    conn.Open();
                    //execute the query (scalar) - áéöåò äùàéìúà
